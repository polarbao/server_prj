...
{
	LOG_INFO(QString(u8"贴合业务线程处理任务: %1 for device %2")
		.arg(QString::fromStdString(task.proId))
		.arg(QString::fromStdString(task.devId)));

	//PerformSimulateTaskInThread(task);

	LOG_INFO(QString(u8"贴合业务线程处理任务: %1 for device %2，完成任务操作，返回服务器信息。")
		.arg(QString::fromStdString(task.proId))
		.arg(QString::fromStdString(task.devId)));
}
...
void StickWorkThdImpl::PerformSimulateTaskInThread(const BusinessTask& inTask)
{
	try
	{
		LOG_INFO(QString(u8"贴合业务线程处理任务_模拟数据: %1 for device %2")
		...
		if (it != m_devicesMap.end())
		{
			...
			if (ret)
			{
				LOG_INFO(QString(u8"贴合业务线程处理任务完成_模拟数据: %1 for device %2")
					.arg(QString::fromStdString(task.proId))
					.arg(QString::fromStdString(task.devId)));
			}
			else if (!ret && it->second->GetIsTaskCancel())
			{
				LOG_INFO(QString(u8"贴合业务线程处理任务完成_模拟数据，服务单取消返回op5: %1 for device %2")
					.arg(QString::fromStdString(task.proId))
					.arg(QString::fromStdString(task.devId)));
	...
	{
		LOG_INFO(QString(u8"贴合任务执行异常: %1").arg(e.what()));
	}
}
...
void StickWorkThdImpl::HandleSimulateData(const BusinessTask& task)
{
	...
	//{
	//	LOG_INFO(QString(u8"制作业务线程定时器到期，已完成下载文件操作，操作结果为%1").arg(ret));
	//});
	//
...
{
	LOG_INFO(QString(u8"贴合任务完成: Device=%1")
...
{
	LOG_INFO(QString(u8"贴合任务错误: Device=%1, Error=%2")
	...
	if (!taskId.empty())
	{
		if (m_taskStatusCallBack1)
		{
			...
			m_taskStatusCallBack1(taskStatus); // 4表示终止服务，重新进入排队态
		}
