...
WorkThdMgrImpl::WorkThdMgrImpl()
{
	...
	// 注册业务线程的任务和设备状态回调 (现在通过信号转发)
	for (const auto& pair : m_workThd) 
	{
		//op 1开始task 2结束task 3取消task 4
...
{
	// 查找设备所属的业务线程并取消任务
	// 1203_debug_note: 设备模拟故障时，进行取消任务操作，使当前任务重新进入排队态
	for (const auto& pair : m_workThd)
	{
