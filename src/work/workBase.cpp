...
void WorkThdBaseImpl::PerformTaskInThread(const BusinessTask& task)
{
	...
	/*
		//op为1时，任务正常结束，opRes上报为3
		//op为1时，设备故障，opRes上报为4

		//op为2时，取消任务，opRes上报为5
	*/
	...
	if (it != m_devicesMap.end())
	{
		...
		{
			taskSuccess = it->second->PerformTask(task); // 执行任务
		} 
		else
		{
			taskSuccess = it->second->PerformSimulateTask(task); // 执行任务
		...
		if (m_taskStatusCallBack1) 
		{
			...
			//设备完成状态改变->空闲态
			//0926: 取消任务时，判断设备是否为故障状态，是则进行设备状态同步；否则设置设备空闲状态
			...
			//0911 finalStatus -> op(任务操作类型 1开始 2结束 3取消认为，进程重新排队 4终止任务
			//完成任务执行
