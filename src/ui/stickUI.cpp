...
StickUI::StickUI(QWidget *parent)
    : QWidget(parent)
...
void StickUI::Init()
{
	...
	//connect(m_devRegTimer, &QTimer::timeout, this, &StickUI::OnSendDevReg);
...
void StickUI::InitConnect()
{
	connect(m_btnGroup, QOverload<QAbstractButton*>::of(&QButtonGroup::buttonClicked), this, &StickUI::OnConnBtnClicked);


}


void StickUI::GetDevStatusInfo(const DeviceStatus status, QString& devStatus, QColor& devStatusColor)
{
	switch (status)
	{
...
void StickUI::SyncRegDevStatus(const std::vector<DeviceInfo>& data)
{
	//Note: 同步设备状态，任务状态
...
void StickUI::SyncWorkDevStatus(const DeviceInfo& data)
{
	//判断设备所处于的slot
	int slotIdx = 0;
...
void StickUI::UpdateDevInfo(int idx, const DeviceInfo& devInfo)
{
...
void StickUI::UpdateDevStatusInfo(QLabel* lab, const DeviceStatus& data)
{
	if (!lab)
	{
		return;
...
void StickUI::OnConnBtnClicked(QAbstractButton* btn)
{
	//获取按钮触发对应的Btn状态
	auto a1 = btn->text();
